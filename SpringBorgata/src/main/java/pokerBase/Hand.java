...
public class Hand {
	...
	private boolean Ace;
	private static Deck regDeck = new Deck();

	private ArrayList<Hand> PossHands = new ArrayList<Hand>();
	...
	private static ArrayList<Hand> genHands(Hand h) {
		ArrayList<Hand> HandsToReturn = new ArrayList<Hand>();
		HandsToReturn.add(h);
		for (int i = 0; i < h.CardsInHand.size(); i++) {
			HandsToReturn = SubHand(HandsToReturn, i);
		}
	...
	private static ArrayList<Hand> SubHand(ArrayList<Hand> inHands, int CardSubbed){

		ArrayList<Hand> SubHands = new ArrayList<Hand>();
		
		for (int i = 0; i < inHands.size(); i++){
			ArrayList<Card> cards = inHands.get(i).getCards();
			for (Card CardToCheck: cards){
				if (CardToCheck.getWild() == true || (CardToCheck.getRank().getRank() == eRank.JOKER.getRank())){
					for (Card WildSub : regDeck.getCards()){
						Hand h = new Hand();
						cards.remove(i);
						cards.add(WildSub);
						Card wild = new Card(WildSub.getSuit(), WildSub.getRank(),0);
						h.AddCardToHand(wild);
						for (int j = 1; j <= cards.size(); j++){
							h.AddCardToHand(cards.get(j));
						}
						SubHands.add(h);
					}
				}
	...
	public static Hand EvalHands(Hand h) {
		...
		ArrayList<Hand> EvalHands = genHands(h);
